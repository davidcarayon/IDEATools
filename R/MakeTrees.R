...
#' @importFrom magrittr %>%
#' @importFrom dplyr mutate rowwise ungroup filter inner_join arrange
  ...
  draw_trees <- function(IDEAdata, nom) {
    ...
    for (prop in c(names(IDEAdata$nodes), "Global_zoom")) {
      liste_indicateurs_prop <- switch(prop,
        ...
        "Robustesse" = c("A1", "A3", "A4", "A14", "C5", "C4", "C7", "A2", "C8", "C9", "A15", "B22", "B18", "B15", "B13", "B16", "R1", "R2", "R3", "R4", "R5", "R6", "R7", "R8", "R9", "R10"),
        ...
        "Global" = c(
          ...
          "A1", "A3", "A4", "A14", "C5", "C4", "C7", "A2", "C8", "C9", "A15", "B22", "B18", "B15", "B13", "B16", "R1", "R2", "R3", "R4", "R5", "R6", "R7", "R8", "R9", "R10",
      ...
      rect_no <- which(str_detect(car, "    <rect") == TRUE)[1:span_rect]

      tab_rect <- tibble(rect_no) %>%
        mutate(rect_end = rect_no + 7) %>%
        mutate(rect_id = map2_dbl(.x = rect_no, .y = rect_end, .f = find_pos, choice = "id", car = car)) %>%
        mutate(rect_style = map2_dbl(.x = rect_no, .y = rect_end, .f = find_pos, choice = "style", car = car)) %>%
        rowwise() %>%
        mutate(rect_number_p = as.numeric(str_extract_all(car[rect_id], "[0-9]+")[[1]][1])) %>%
        mutate(rect_number_c = as.numeric(str_extract_all(car[rect_id], "[0-9]+")[[1]][2])) %>%
        ungroup() %>%
      ...
      tab_to_color <- IDEAdata$nodes[[filter_prop]] %>%
        filter(id_exploit == nom) %>%
        gather(key = indicateur, value = resultat, -id_exploit) %>%
        mutate(indicateur = replace_indicateur(indicateur)) %>%
        inner_join(tab_rect, by = "indicateur") %>%
        mutate(color = replace_col(resultat)) %>%
        arrange(rect_number_p, rect_number_c)

      for (i in which(tab_to_color$color == "lightgreen")) {
        car[rect_style[i]] <- str_replace(car[rect_style[i]], "fill:#ffffff", "fill:#1CDA53")
      }

      for (i in which(tab_to_color$color == "green")) {
        car[rect_style[i]] <- str_replace(car[rect_style[i]], "fill:#ffffff", "fill:#0D8A00")
      }

      for (i in which(tab_to_color$color == "tomato")) {
        car[rect_style[i]] <- str_replace(car[rect_style[i]], "fill:#ffffff", "fill:#FF6348")
      }

      for (i in which(tab_to_color$color == "red")) {
        car[rect_style[i]] <- str_replace(car[rect_style[i]], "fill:#ffffff", "fill:#FF0000")
      }

      for (i in which(tab_to_color$color == "orange")) {
        car[rect_style[i]] <- str_replace(car[rect_style[i]], "fill:#ffffff", "fill:#FFA300")
      }

      for (i in which(tab_to_color$color == "grey")) {
        car[rect_style[i]] <- str_replace(car[rect_style[i]], "fill:#ffffff", "fill:#A0A0A0")
      }
