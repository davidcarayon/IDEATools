#' Render a PDF/HTML report of an IDEA diagnosis
#'
...
#' MakePagedownReport(path)
MakePagedownReport <- function(input, output_dir = getwd(), silent = FALSE) {
  ...
  template <- system.file("report/rapport_individuel_pagedown.Rmd", package = "IDEATools")
  ...
  output_file <- paste0(file,"_",basename(tools::file_path_sans_ext(input)),".html")
