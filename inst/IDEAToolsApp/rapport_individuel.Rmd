...
knitr::kable(metadata %>% tidyr::gather(key = code, value = valeur) %>% dplyr::inner_join(MTD_legende, by ="code") %>% dplyr::select(Métadonnée = nom, Valeur = valeur) %>% dplyr::slice(1:7))
