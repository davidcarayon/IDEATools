...
server = function(input, output, session) {
  ...
  IDEAdata <- eventReactive(input$files, {
    ...
    IDEATools::importIDEA2(input = input$files$datapath, anonymous = FALSE)
  ...
  observeEvent(input$files, {
    withProgress(message = "Coloration des cartes heuristiques...", detail = "Merci de patienter quelques instants", value = 0.2, {
      ...
      IDEATools::MakeTrees2(IDEAdata()) %>% IDEATools::exportIDEA2(outdir = outdir)
      incProgress(0.8)})
  ...
  output$zipfile <- downloadHandler(
    ...
    content = function(fname) {
      ...
      withProgress(message = "Production des figures en cours........", detail = "Merci de patienter quelques instants", value = 0.2,{
        ...
        v <- IDEAdata()$metadata$MTD_01 %>%  stringr::str_replace_all(" ", "_")
        IDEAresdim() %>% IDEATools::exportIDEA2(outdir = outdir)
        IDEAresrad() %>% IDEATools::exportIDEA2(outdir = outdir)
        IDEATools::MakeTrees2(IDEAdata()) %>% IDEATools::exportIDEA2(outdir = outdir)
