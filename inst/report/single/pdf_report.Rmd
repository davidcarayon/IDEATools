...
tibble::tibble(metadata_code = names(IDEA_data$metadata), value = unlist(IDEA_data$metadata)) %>% 
  dplyr::inner_join(IDEATools:::reference_list$metadata, by = "metadata_code") %>% 
  dplyr::select(`Caractéristique` = metadata_name, Valeur = value) %>% 
  knitr::kable()
```
...
```{r fig.cap= "Résultats pour les 13 composantes", fig.width = 14.5, fig.height=15}
