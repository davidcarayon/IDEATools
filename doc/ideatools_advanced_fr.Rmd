...
Il est aussi nécessaire d'installer [LaTeX](latex-project.org) pour la production d'un rapport PDF. Il est possible d'installer une version très allégée (mais suffisante) de LaTeX : [tinytex](https://yihui.org/tinytex/).

Il est tout de même possible de produire un PDF sans installer LaTeX. Il suffit de produire un rapport au format HTML, puis de l'enregistrer au format PDF via l'interface d'impression d'un navigateur web tel que Google Chrome.
...
-   Si le format est html, alors c'est {pagedown} qui est utilisé pour produire des rapports HTML paginés. Les feuilles de style (css) sont stockées dans les fichiers internes du package
