...
Si vous souhaitez utiliser certaines fonctionnalités avancées (interface graphique, rapports automatisés), vous des packages supplémentaires sont nécessaires, il peuvent être installés directement en préciseant l'argument `dependencies` : 

```{r}
remotes::install_github("davidcarayon/IDEATools", dependencies = TRUE)
```

_NB : Le logiciel RTools est parfois nécessaire sur les machines Windows. Vous pouvez l'installer ici : [Rtools](https://cran.r-project.org/bin/windows/Rtools/)_
...
- Un module intitulé `jsonify()` qui permet de convertir un calculateur excel IDEA en un fichier JSON, notamment utilisé par le WEBIDEA, dans le cas où la macro excel du calculateur ne fonctionnerait pas.
...
Carayon D (2022). IDEATools: A Collection of Tools Dedicated to the IDEA4 Method. R package version 3.3.0.
