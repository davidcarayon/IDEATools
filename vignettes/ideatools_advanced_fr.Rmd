...
vignette: >
  %\VignetteIndexEntry{Historique de développement}
...
Cette version apporte quelques évolutions majeures dans le code du package ainsi que ses fonctionnalités. Elle apporte notamment une interface graphique utilisateur très basique et accessible d'un simple `runGUI()` pour simplifier l'utilisation du package pour des utilisateurs ayant installé R uniquement pour utiliser le package. Dans un but de publier un jour le package sur le CRAN, d'importants efforts ont été menés sur la réduction du nombre de dépendances (retrait des pipes magrittr au profit du pipe de R 4.1.0, retrait des messages un peu stylisés en console, retrait de map au profit des apply, passage en Suggest de nombreux packages bonus, etc).
...
Le package R {IDEATools} nécessite en premier lieu l'installation de [R](https://cran.r-project.org/) puis de [Rstudio](https://www.rstudio.com/products/rstudio/download/).
...
| MTD\_17    | Lycée agricole: oui /non             | Texte     |
